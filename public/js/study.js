...
export async function openNewStudy(q = {}) {
  ...
  const m = modal(`<h2>New study set</h2>
    <div class="field"><label>Test / topic</label><input type="text" id="stTitle" value="${esc(ev ? ev.title : q.topic ? q.topic : (preNb ? (nbs.find(n => n.id === preNb)?.name || '') : ''))}" placeholder="e.g. Chapter 5 test — Cells"></div>
...
function planTab(s, body) {
  if (!s.plan) {
    ...
    const ppdDef = state.user?.settings?.planMinutes || 30; const ppdSel = $('#ppd'); if (ppdSel && [...ppdSel.options].some(o => +o.value === ppdDef)) ppdSel.value = String(ppdDef);
